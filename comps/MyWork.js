...
const projects = [
  {
    ...
    pic: "/pinningBees.png",
    description: [
      "A fully responsive, light-weight Front-end site designed with minimal dependencies (jQuery, TS). ",
      "Fluid animations add an interactive feel, responsive design implemented tastefully, and a fully functioning contact form.",
      "All completed in 3 days from start-to-finish, idea to deployment.",
    ],
    links: {
      github: "https://github.com/evanoshea21/pinning-bees",
      web: "http://pinningbees.com",
    },
  },
  {
    ...
    pic: "/qaAPI.jpg",
    description: [
      "A Deployed Backend RESTful API ready for real-world traffic spikes of up to 10,000 clients/second.",
      "Deployed NginX Load Balancer to AWS EC2 instance to increase throughput from 3200 clients/sec to over 10k clients/sec, dropping error rate from 12% to 0.0% and response time reduced from 16sec to 130ms.",
    ],
  ...
  },
  {
    ...
    description: [
      ...
      "Responsible for the Question and Answers section.",
      "Utilized localStorage for caching, jquery scroll animations for infinite scrolling, and created a real-time search bar with React onChange hook.",
    ],
  ...
  },
  {
    ...
    pic: "/techX2.png",
    description: [
      "A React-Native mobile app that facilitates electronic device trades between users.",
      "Introduced global state with React Context API to pre-load data and reduce redundant http requests.",
      "Full Auth process with photo uploads implemented with Firebase and S3 buckets.",
    ],
        ...
        {projects.map((card, i) => {
          return (
            <div
              ...
              className={`${classes.card} cardHover card${i}`}
            >
          ...
          return (
            <div key={`${i}smallCard`} className={classes.cardSmall}>
              ...
              <div className={classes.infoSm}>
                <h2 className={`${classes.projTitle} ${classes.titleSm}`}>
                  {card.title}
                </h2>
                <div className={`${classes.role} ${classes.roleSm}`}>
                  {card.role}
                </div>
                <div className={classes.desc}>
                  {card.description.map((para, i) => {
